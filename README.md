...
**Before authoring an MSB:** read [Authoring Movie Source Bundles](docs/msb-authoring.md). Entity-level character, location, and prop references are packaged source documentation; they are not automatically composited into provider input. Each shot's `references` is a role-based object — `composition` (one starting-frame raster), `identity` (zero to three character rasters), and `endFrame` (one optional ending-frame raster) — and the configured renderer's `mode` determines which roles are accepted and how many. Image-to-video engines upload the single `composition` raster; the Veo 3.1 Fast reference-to-video engine uploads one to three `identity` rasters instead. Shots are generated independently, and `continuity` is prompt guidance rather than an identity lock or previous-frame handoff.
