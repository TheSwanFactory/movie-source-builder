...
- `formatVersion` identifies the `.msbc` schema, not the renderer version.
...
The schema is [`schemas/msbc-configuration.schema.json`](../schemas/msbc-configuration.schema.json). The top-level object is strict, so content-specific or misspelled fields are rejected.
