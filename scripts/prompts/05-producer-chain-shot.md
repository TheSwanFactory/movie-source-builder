...
For a shot that should continue directly from an earlier one — same set,
continuous framing, no reason to re-invent the composition, including any
shot the Author just split off an earlier one in the previous step — add
`"chainFrom": "<earlier-shot-id>"` to it in `msb.json` before packing —
`image-to-video` only. This has to happen now, before the bundle is packed;
`chainFrom` set after packing or rendering has already happened is too late.
